...
/*
 * -------------------- iterate over even points (Sequential/OMP version) --------------------
 */
...
//    if (1 == flag)
//    {
//        string strTestFile = "rank1_solver_itrEvenPoints.dat";
...
/*
 * -------------------- iterate over even points (MPI version) --------------------
 */
void CDelphiFastSOR::mpi_itrEvenPoints(const int& forWhom, const int& flag)
{
    delphi_integer n, ix, iy, iz;
    delphi_integer star, fin;
    delphi_real temp1, temp2, temp3, temp4;
    ...
    if (0 == mpi_rank) /* master process */
    {
        ...
        if (rgbPeriodicBndy[0] || rgbPeriodicBndy[1] || rgbPeriodicBndy[2])
        {
            ...
            MPI_Scatterv(mpi_phimap1, mpi_sendcounts1l, mpi_senddispls1l, mpi_delphi_real, 
                         MPI_IN_PLACE, 0, mpi_delphi_real, 0, MPI_COMM_WORLD);
            MPI_Scatterv(mpi_phimap1, mpi_sendcounts1r, mpi_senddispls1r, mpi_delphi_real, 
                         MPI_IN_PLACE, 0, mpi_delphi_real, 0, MPI_COMM_WORLD);
        }
        ...
        if (rgbPeriodicBndy[0] || rgbPeriodicBndy[1] || rgbPeriodicBndy[2])
        {
            ...
            MPI_Gatherv(MPI_IN_PLACE, 0, mpi_delphi_real, 
                        mpi_phimap2, mpi_recvcounts2, mpi_recvdispls2, mpi_delphi_real, 
                        0, MPI_COMM_WORLD);
    ...
    else
    {
        ...
        if (rgbPeriodicBndy[0] || rgbPeriodicBndy[1] || rgbPeriodicBndy[2])
        {
            ...
            MPI_Scatterv(mpi_phimap1 + mpi_wrphimap1_start, mpi_recvcounts1, mpi_recvdispls1, mpi_delphi_real, 
                         mpi_phimap1 + mpi_wrstar2[mpi_rank] - (iGrid * iGrid - 1) / 2 - 1, mpi_wrstar1[mpi_rank] - (mpi_wrstar2[mpi_rank] - (iGrid * iGrid - 1) / 2), mpi_delphi_real, 
                         0, MPI_COMM_WORLD);
            MPI_Scatterv(mpi_phimap1 + mpi_wrphimap1_start, mpi_recvcounts1, mpi_recvdispls1, mpi_delphi_real,
                         mpi_phimap1 + mpi_wrfinl1[mpi_rank], mpi_wrfinl2[mpi_rank] + (iGrid * iGrid + 1) / 2 - mpi_wrfinl1[mpi_rank], mpi_delphi_real, 
                         0, MPI_COMM_WORLD);
        }
        /*
        ...
        if (fZero < abs(fIonStrength))  //----- the main loop is as below:
        {
            for (n = 1; n <= mpi_wrnstafi2[mpi_rank]; n++)
            {
                star = sta2[n];
                fin  =  fi2[n];
                for (ix = star; ix <= fin; ix++)
                {
                    temp1 = mpi_phimap1[ ix - 1         ] + mpi_phimap1[(ix - 1) + 1    ];
                    temp2 = mpi_phimap1[(ix - 1) + lat2 ] + mpi_phimap1[(ix - 1) - lat1 ];
                    temp3 = mpi_phimap1[(ix - 1) + long2] + mpi_phimap1[(ix - 1) - long1];
                    mpi_phimap2[ix - 1] = mpi_phimap2[ix - 1] * om1 + (mpi_qmap2[ix - 1] + temp1 + temp2 + temp3) * mpi_sf2[ix - 1];
                }
            }
        }
        else //----- if there is no salt then the main loop is executed without sf saving about 15% in execution time
        {
            for (n = 1; n <= mpi_wrnstafi2[mpi_rank]; n++)
            {
                ...
                for (ix = star; ix <= fin; ix++)
                {
                    ...
                    temp2 = mpi_phimap1[(ix - 1) + lat2 ] + mpi_phimap1[(ix - 1) - lat1 ];
                    temp3 = mpi_phimap1[(ix - 1) + long2] + mpi_phimap1[(ix - 1) - long1];
                    mpi_phimap2[ix - 1] = mpi_phimap2[ix - 1] * om1 + (temp1 + temp2 + temp3) * sixth;
                }
            }
        }
...
//          }           
        ...
        {
            for (n = mpi_wricount2a; n < mpi_wricount2b; n++)
            {
                ix = prgiBndyDielecIndex[n];

                temp1 = mpi_phimap1[(ix - 1)        ] * prgfBndyDielec[n][0] + mpi_phimap1[ ix             ] * prgfBndyDielec[n][1];
                temp2 = mpi_phimap1[(ix - 1) - lat1 ] * prgfBndyDielec[n][2] + mpi_phimap1[(ix - 1) + lat2 ] * prgfBndyDielec[n][3];
                temp3 = mpi_phimap1[(ix - 1) - long1] * prgfBndyDielec[n][4] + mpi_phimap1[(ix - 1) + long2] * prgfBndyDielec[n][5];
                mpi_phimap2[(ix - 1)] += temp1 + temp2 + temp3;
            }
        }
        ...
        for (n = 0; n < fin - star + 1; n++) 
        {
            ...
            if (mpi_wrphimap2_start <= (iy - 1) && (iy - 1) <= mpi_wrphimap2_end)
                mpi_phimap2[iy - 1] = bndx3[n];
            
            if (mpi_wrphimap2_start <= (iy + ((iGrid + 1) / 2 - 1) - 1) && (iy + ((iGrid + 1) / 2 - 1) - 1) <= mpi_wrphimap2_end)
                mpi_phimap2[iy + ((iGrid + 1) / 2 - 1) - 1] = bndx4[n];
        }
...
//              }
            ...
            {
                for (n = mpi_wricount1a; n < mpi_wricount1b; n++)
                {
                    ix = prgiCrgPose[n];
                    mpi_phimap2[ix - 1] += prgfCrgValA[n];
                }
            }           
        }
        ...
        if (rgbPeriodicBndy[0] || rgbPeriodicBndy[1] || rgbPeriodicBndy[2])
        {
            MPI_Gatherv( mpi_phimap2 + mpi_wrstar2[mpi_rank] - 1, mpi_wrlen2[mpi_rank], mpi_delphi_real, 
                         mpi_phimap2 + mpi_wrphimap2_start, mpi_recvcounts2, mpi_recvdispls2, mpi_delphi_real, 0, MPI_COMM_WORLD);
            MPI_Scatterv(mpi_phimap2 + mpi_wrphimap2_start, mpi_recvcounts2, mpi_recvdispls2, mpi_delphi_real, 
                         mpi_phimap2 + mpi_wrstar2[mpi_rank] - 1, mpi_wrlen2[mpi_rank], mpi_delphi_real, 0, MPI_COMM_WORLD);
        }
...
//        string strTestFile;
//        
//        if (0 == mpi_rank) strTestFile = "rank0_solver_itrEvenPoints.dat";
//        if (1 == mpi_rank) strTestFile = "rank1_solver_itrEvenPoints.dat";
//        if (2 == mpi_rank) strTestFile = "rank2_solver_itrEvenPoints.dat";
//        if (3 == mpi_rank) strTestFile = "rank3_solver_itrEvenPoints.dat";
//        
//        ofstream ofTestStream(strTestFile.c_str());
//        ofTestStream << boolalpha;
//        ofTestStream << fixed << setprecision(7);
