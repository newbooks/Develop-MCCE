#ifndef DDM__TILE_PATTERN_H_
#define DDM__TILE_PATTERN_H_
...
#include "../../ddm/pattern/internal/PatternArguments.h"
...
private:
  ...
  SizeType                    _nunits          = ddm::Team::All().size();
...
public:
   ...
   *   auto num_units = ddm::Team::All.size();
   *   // Same as
  ...
  {
    DDM_LOG_TRACE("TilePattern()", "Constructor with Argument list");
  ...
  : _distspec(dist),
    ...
    _local_capacity(
        initialize_local_capacity(_local_memory_layout))
  {
    DDM_LOG_TRACE("TilePattern()", "(sizespec, dist, teamspec, team)");
    initialize_local_range();
  }
  ...
  : _distspec(dist),
    ...
    _local_capacity(
        initialize_local_capacity(_local_memory_layout))
  {
    DDM_LOG_TRACE("TilePattern()", "(sizespec, dist, team)");
    initialize_local_range();
  }
  ...
  {
    DDM_LOG_TRACE("TilePattern.unit_at()",
                   "coords:",   coords,
                   "viewspec:", viewspec);
    ...
    DDM_LOG_TRACE_VAR("TilePattern.unit_at", block_coords);
    DDM_LOG_TRACE_VAR("TilePattern.unit_at", unit_ts_coords);
    DDM_LOG_TRACE_VAR("TilePattern.unit_at >", unit_id);
    return unit_id;
  }
  ...
  IndexType local_at(
    ...
    const ViewSpec_t & viewspec) const
  {
    DDM_LOG_TRACE("TilePattern.local_at()",
    ...
    DDM_LOG_TRACE_VAR("TilePattern.local_at >", local_index);
    return local_index;
  }
  ...
  {
    DDM_LOG_TRACE_VAR("TilePattern.local()", g_index);
  ...
  local_index_t local_index(
    const std::array<IndexType, NumDimensions> & global_coords) const
  {
    DDM_LOG_TRACE_VAR("TilePattern.local_index()", global_coords);
    ...
    DDM_LOG_TRACE_VAR("TilePattern.local_index", l_coords);
    DDM_LOG_TRACE_VAR("TilePattern.local_index", unit);
    ...
    if (unit == _myid) {
      ...
      DDM_LOG_TRACE("TilePattern.local_index",
    ...
    DDM_LOG_TRACE_VAR("TilePattern.local_index >", l_index);
  ...
  std::array<IndexType, NumDimensions> global(
    ...
    const std::array<IndexType, NumDimensions> & local_coords) const
  {
    DDM_LOG_DEBUG("TilePattern.global()",
                   "unit:",    unit,
    ...
    DDM_LOG_DEBUG_VAR("TilePattern.global >", global_coords);
  ...
  {
    DDM_LOG_TRACE("TilePattern.global()",
    ...
    DDM_LOG_TRACE_VAR("TilePattern.global >", offset);
    return offset;
  }
  ...
  IndexType global_index(
    ...
    const std::array<IndexType, NumDimensions> & local_coords) const
  {
    DDM_LOG_TRACE("TilePattern.global_index()",
                   "unit:",         unit,
                   "local_coords:", local_coords);
    std::array<IndexType, NumDimensions> global_coords =
      global(unit, local_coords);
    auto g_index = _memory_layout.at(global_coords);
    DDM_LOG_TRACE_VAR("TilePattern.global_index >", g_index);
    return g_index;
  }
  ...
  IndexType global_at(
    ...
    const ViewSpec_t                           & viewspec) const
  {
    DDM_LOG_TRACE("TilePattern.global_at()",
    ...
    auto offset = block_index * _blocksize_spec.size() + // preceed. blocks
                  _blocksize_spec.at(phase_coords);      // element phase
    DDM_LOG_TRACE_VAR("TilePattern.global_at >", offset);
    return offset;
  }
    ...
    const ViewSpec_t                           & viewspec) const
  {
    ...
    auto offset = l_block_index * _blocksize_spec.size() + // prec. blocks
                  _blocksize_spec.at(phase_coords);        // elem. phase
    DDM_LOG_TRACE_VAR("TilePattern.at >", offset);
    return offset;
  }
  ...
  {
    DDM_LOG_TRACE("TilePattern.at()", "gcoords:",  global_coords);
    // Phase coordinates of element:
    std::array<IndexType, NumDimensions> phase_coords;
    // Coordinates of the block containing the element:
    std::array<IndexType, NumDimensions> block_coords;
  ...
  bool has_local_elements(
    ...
    const ViewSpec_t & viewspec) const
  {
    DDM_LOG_TRACE_VAR("TilePattern.has_local_elements()", dim);
    DDM_LOG_TRACE_VAR("TilePattern.has_local_elements()", dim_offset);
    DDM_LOG_TRACE_VAR("TilePattern.has_local_elements()", unit);
    DDM_LOG_TRACE_VAR("TilePattern.has_local_elements()", viewspec);
    ...
    team_unit_t unit) const
  {
    ...
    DDM_LOG_TRACE_VAR("TilePattern.is_local >", (coords_unit == unit));
    ...
    const std::array<index_type, NumDimensions> & g_coords) const
  {
    ...
    DDM_LOG_TRACE("TilePattern.block_at",
                   "coords", g_coords,
    ...
    const std::array<index_type, NumDimensions> & g_coords) const
  {
    ...
    DDM_LOG_TRACE("TilePattern.local_block_at >",
                   "coords",             g_coords,
                   "unit:",              l_pos.unit,
  ...
  {
    DDM_LOG_TRACE_VAR("TilePattern.block()", global_block_index);
    // block index -> block coords -> offset
    auto block_coords = _blockspec.coords(global_block_index);
    DDM_LOG_TRACE_VAR("TilePattern.block", block_coords);
    DDM_LOG_TRACE_VAR("TilePattern.block", _blocksize_spec.extents());
    ...
    DDM_LOG_TRACE("TilePattern.block",
                   "offsets:", offsets,
                   "extents:", extents);
    auto block_vs = ViewSpec_t(offsets, extents);
    DDM_LOG_TRACE_VAR("TilePattern.block >", block_vs);
    return block_vs;
  }
  ...
  ViewSpec_t local_block(
    ...
    index_type   local_block_index) const
  {
    DDM_LOG_TRACE("TilePattern.local_block()",
                   "unit:",       unit,
    ...
    DDM_LOG_TRACE_VAR("TilePattern.local_block", l_block_coords);
    std::array<index_type, NumDimensions> offsets;
    std::array<size_type, NumDimensions>  extents;
    for (auto d = 0; d < NumDimensions; ++d) {
      auto blocksize_d = _blocksize_spec.extent(d);
    ...
    DDM_LOG_TRACE_VAR("TilePattern.local_block >", block_vs);
    return block_vs;
  }
  ...
  ViewSpec_t local_block_local(
    index_type local_block_index) const
  {
    DDM_LOG_TRACE_VAR("TilePattern.local_block_local()",
                       local_block_index);
    ...
    ViewSpec_t block_vs(offsets, extents);
    DDM_LOG_TRACE_VAR("TilePattern.local_block_local >", block_vs);
    return block_vs;
  }
...
private:
  ...
  BlockSizeSpec_t initialize_blocksizespec(
    ...
    const TeamSpec_t         & teamspec) const {
    DDM_LOG_TRACE("TilePattern.init_blocksizespec()");
    ...
    for (auto d = 0; d < NumDimensions; ++d) {
      ...
      DDM_ASSERT_GT(extent_d, 0,
                     "Extent of size spec in dimension" << d << "is 0");
      DDM_ASSERT_GT(units_d,  0,
      ...
      DDM_ASSERT_EQ(0, extent_d % blocksize_d,
    ...
    DDM_LOG_TRACE_VAR("TilePattern.init_blocksizespec >", s_blocks);
    return BlockSizeSpec_t(s_blocks);
  }
  ...
  BlockSpec_t initialize_blockspec(
    ...
    const TeamSpec_t         & teamspec) const
  {
    DDM_LOG_TRACE("TilePattern.init_blockspec()",
    ...
    if (teamspec.size() == 0 || sizespec.size() == 0) {
      ...
      DDM_LOG_TRACE_VAR("TilePattern.init_blockspec >",
    ...
    for (auto d = 0; d < NumDimensions; ++d) {
      ...
      SizeType max_blocks_d    = ddm::math::div_ceil(
                                   sizespec.extent(d),
                                   max_blocksize_d);
      n_blocks[d] = max_blocks_d;
    }
    BlockSpec_t blockspec(n_blocks);
    DDM_LOG_TRACE_VAR("TilePattern.init_blockspec >", n_blocks);
    return blockspec;
  }
  ...
  {
    DDM_LOG_TRACE_VAR("TilePattern.init_local_blockspec()",
                       blockspec.extents());
    ...
    DDM_LOG_TRACE_VAR("TilePattern.init_local_blockspec", unit_ts_coords);
    for (auto d = 0; d < NumDimensions; ++d) {
    ...
    DDM_LOG_TRACE_VAR("TilePattern.init_local_blockspec >", l_blocks);
  ...
  {
    DDM_LOG_TRACE_VAR("TilePattern.init_local_capacity()",
                       local_extents.extents());
    auto l_capacity = local_extents.size();
    DDM_LOG_TRACE_VAR("TilePattern.init_local_capacity >", l_capacity);
    return l_capacity;
  }
  ...
  void initialize_local_range()
  {
    ...
    DDM_LOG_DEBUG_VAR("TilePattern.init_local_range()", local_size);
    if (local_size == 0) {
      _lbegin = 0;
      _lend   = 0;
    } else {
    ...
    DDM_LOG_DEBUG_VAR("TilePattern.init_local_range >", _lbegin);
    DDM_LOG_DEBUG_VAR("TilePattern.init_local_range >", _lend);
  }
  ...
  {
    // Coordinates of local unit id in team spec:
    DDM_LOG_DEBUG_VAR("TilePattern.init_local_extents()", unit);
    if (_teamspec.size() == 0) {
      ::std::array<SizeType, NumDimensions> empty_extents = {{ }};
      DDM_LOG_DEBUG_VAR("TilePattern.init_local_extents >", empty_extents);
      return empty_extents;
    }
    auto unit_ts_coords = _teamspec.coords(unit);
    DDM_LOG_TRACE_VAR("TilePattern.init_local_extents", unit_ts_coords);
    ...
    DDM_LOG_DEBUG_VAR("TilePattern.init_local_extents >", l_extents);
    return l_extents;
  }
};
...
#include "../../ddm/pattern/TilePattern1D.h"

#endif // DDM__TILE_PATTERN_H_
