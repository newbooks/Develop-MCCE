#ifndef DDM__TILE_PATTERN_1D_H_
#define DDM__TILE_PATTERN_1D_H_
...
public:
  ...
  : _arguments(arg, args...),
    ...
    DDM_LOG_TRACE("TilePattern<1>()", "Constructor with argument list");
    initialize_local_range();
    DDM_LOG_TRACE("TilePattern<1>()", "TilePattern initialized");
  }
  ...
  : _size(sizespec.size()),
    ...
    _nlblocks(initialize_num_local_blocks(
        ...
        _local_size)),
    _local_capacity(initialize_local_capacity()) {
    DDM_LOG_TRACE("TilePattern<1>()", "(sizespec, dist, teamspec, team)");
    initialize_local_range();
    DDM_LOG_TRACE("TilePattern<1>()", "TilePattern initialized");
  }
  ...
  : _size(sizespec.size()),
    ...
    _nlblocks(initialize_num_local_blocks(
        ...
        _local_size)),
    _local_capacity(initialize_local_capacity()) {
    DDM_LOG_TRACE("TilePattern<1>()", "(sizespec, dist, team)");
    initialize_local_range();
    DDM_LOG_TRACE("TilePattern<1>()", "TilePattern initialized");
  }
  ...
  : _size(other._size),
    ...
    DDM_LOG_TRACE("TilePattern<1>(other)", "TilePattern copied");
  }
  ...
  self_t & operator=(const self_t & other) {
    DDM_LOG_TRACE("TilePattern<1>.=(other)");
    if (this != &other) {
      _size                = other._size;
      _memory_layout       = other._memory_layout;
      _distspec            = other._distspec;
      ...
      _lbegin              = other._lbegin;
      _lend                = other._lend;
      DDM_LOG_TRACE("TilePattern<1>.=(other)", "TilePattern assigned");
    }
    return *this;
  }
  ...
  team_unit_t unit_at(
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.unit_at()", global_pos);
    // Apply viewspec offsets to coordinates:
  ...
  std::array<SizeType, NumDimensions> local_extents(
    ...
    DDM_LOG_DEBUG_VAR("TilePattern<1>.local_extents()", unit);
    DDM_LOG_DEBUG_VAR("TilePattern<1>.local_extents >", _local_size);
  ...
  local_index_t local(
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.local()", g_index);
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.local >", unit);
    index_type  l_index       = (l_block_index * _blocksize) + l_phase;
    DDM_LOG_TRACE_VAR("TilePattern<1>.local >", l_index);
  ...
  local_index_t local_index(
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.local_index()", g_coords);
    index_type  g_block_index = g_coords[0] / _blocksize;
    index_type  l_phase       = g_coords[0] % _blocksize;
    index_type  l_block_index = g_block_index / _nunits;
    team_unit_t unit(g_block_index % _nunits);
    DDM_LOG_TRACE_VAR("TilePattern<1>.local_index >", unit);
    // Global coords to local coords:
    index_type  l_index       = (l_block_index * _blocksize) + l_phase;
    DDM_LOG_TRACE_VAR("TilePattern<1>.local_index >", l_index);
    return local_index_t { unit, l_index };
  }
  ...
  std::array<IndexType, NumDimensions> global(
    team_unit_t unit,
    const std::array<IndexType, NumDimensions> & local_coords) const {
    DDM_LOG_DEBUG_VAR("TilePattern<1>.global()", unit);
    DDM_LOG_DEBUG_VAR("TilePattern<1>.global()", local_coords);
    DDM_LOG_TRACE_VAR("TilePattern<1>.global", _nunits);
    if (_nunits < 2) {
      return local_coords;
    }
    DDM_LOG_TRACE_VAR("TilePattern<1>.global", _nblocks);
    const Distribution & dist = _distspec[0];
    IndexType local_index     = local_coords[0];
    IndexType elem_phase      = local_index % _blocksize;
    DDM_LOG_TRACE_VAR("TilePattern<1>.global", local_index);
    DDM_LOG_TRACE_VAR("TilePattern<1>.global", elem_phase);
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.global", block_index);
    DDM_LOG_TRACE_VAR("TilePattern<1>.global >", glob_index);
  ...
  bool has_local_elements(
    ...
    DDM_ASSERT_EQ(
      ...
      "Expected dimension = 0, got " << dim);
    DDM_LOG_TRACE_VAR("TilePattern<1>.has_local_elements()", dim_offset);
    DDM_LOG_TRACE_VAR("TilePattern<1>.has_local_elements()", unit);
    DDM_LOG_TRACE_VAR("TilePattern<1>.has_local_elements()", viewspec);
  ...
  bool is_local(
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.is_local >", (coords_unit == unit));
  ...
  BlockSpec_t blockspec() const {
    BlockSpec_t bspec({ ddm::math::div_ceil(_size, _blocksize) });
    return bspec;
  }
    ...
    const std::array<index_type, 1> & g_coords) const
  {
    ...
    DDM_LOG_TRACE("TilePattern<1>.block_at",
                   "coords", g_coords,
  ...
  {
    DDM_LOG_DEBUG_VAR("TilePattern<1>.local_block()", l_block_index);
    ...
    DDM_LOG_DEBUG_VAR("TilePattern<1>.local_block >", block_vs);
    return block_vs;
  }
  ...
  ViewSpec_t local_block_local(
    index_type l_block_index) const
  {
    DDM_LOG_DEBUG_VAR("TilePattern<1>.local_block_local()", l_block_index);
    index_type offset = l_block_index * _blocksize;
    std::array<index_type, NumDimensions> offsets = {{ offset }};
    std::array<size_type, NumDimensions>  extents = {{ _blocksize }};
    ViewSpec_t block_vs(offsets, extents);
    DDM_LOG_DEBUG_VAR("TilePattern<1>.local_block_local >", block_vs);
    return block_vs;
  }
  ...
  SizeType initialize_blocksize(
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_blocksize", nunits);
    if (nunits == 0) {
      return 0;
    }
  ...
  SizeType initialize_num_blocks(
    ...
    DDM_LOG_TRACE("TilePattern<1>.init_num_blocks()",
                   "size", size, "blocksize", blocksize, "nunits", nunits);
    SizeType n_blocks = ddm::math::div_ceil(
                          size,
                          blocksize);
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_blockspec", n_blocks);
    return n_blocks;
  }
  ...
  SizeType initialize_num_local_blocks(
    ...
    if (blocksize > 0) {
      num_l_blocks = ddm::math::div_ceil(
                       num_l_blocks,
                       blocksize);
    } else {
      num_l_blocks = 0;
    }
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_num_local_blocks", num_l_blocks);
    return num_l_blocks;
  }
  ...
  SizeType initialize_local_capacity() const {
    ...
    auto max_l_blocks = ddm::math::div_ceil(
                          _nblocks,
                          _nunits);
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_lcapacity.d", _nunits);
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_lcapacity.d", max_l_blocks);
    l_capacity = max_l_blocks * _blocksize;
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_lcapacity >", l_capacity);
    return l_capacity;
  }
  ...
  void initialize_local_range() {
    ...
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_local_range()", l_size);
    if (l_size == 0) {
      _lbegin = 0;
      _lend   = 0;
    } else {
    ...
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_local_range >", _lbegin);
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_local_range >", _lend);
  }
  ...
  SizeType initialize_local_extent(
    team_unit_t unit) const {
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_local_extent()", unit);
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_local_extent()", _nunits);
    if (_nunits == 0) {
      return 0;
    }
    ...
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_local_extent", _nblocks);
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_local_extent", _blocksize);
    DDM_LOG_TRACE_VAR("TilePattern<1>.init_local_extent", min_local_blocks);
    l_extent = min_local_blocks * _blocksize;
    DDM_LOG_DEBUG_VAR("TilePattern<1>.init_local_extent >", l_extent);
    return l_extent;
  }
};
...
#endif // DDM__BLOCK_PATTERN_1D_H_
